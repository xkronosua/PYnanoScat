...
class nanoScat_PD(QtGui.QMainWindow):
	...
	def openAngleSensorPort(self,state):
		if state:
			...
			port = 'COM'+str(self.ui.angleSensorPort.value())
			self.angleSensorSerial = serial.Serial(port,baudrate=19200)#.port = port
			print(port)
			...
			a=self.angleSensorSerial.read(20)
			print(a)
	...
	def getNanoScatInfo(self,keys=['angle']):
		if not self.angleSensorSerial.isOpen():
			...
			self.angleSensorSerial.flushInput()
			self.angleSensorSerial.flushOutput()
			line = self.angleSensorSerial.readline(30).decode("utf-8")
			print(line)
			if 'angle' in keys:
				try:
